...
Use **Decap CMS** (a Git-based CMS) as the primary editorial interface for case studies and key profile/site settings information, with content stored in Markdown/JSON/YAML files in the repository. Site settings and configuration also live in version control.
...
- No separate hosted CMS infrastructure is required beyond GitHub and the static site hosting.

## Consequences

- Requires configuration of Decap CMS (admin UI, authentication with GitHub, and collection definitions for case studies, footer company logos, and profile data).
